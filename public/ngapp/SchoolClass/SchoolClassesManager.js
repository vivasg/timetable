...
function SchoolClassesManager() {
    var self = this;
    var SchoolClassesManagerClass = function ($http, $q) {
        self.$http = $http;
        self._cashPool = {};
    };

    /** METHODS */
    SchoolClassesManagerClass.prototype = {
            ...
            self.$http.post("'api.php?controller=school_rooms&action=item&name=" + name) //wrong address
                .then(function (responce) {
                    if (responce.status >= 200 && responce.status < 300) { //JSON.parse(responce)
                        var instance = new SchoolClass(responce.data);
    ...
    return SchoolClassesManagerClass;
}
