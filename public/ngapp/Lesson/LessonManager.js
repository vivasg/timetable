...
function LessonManager($http, $q) {
    var self = this;
    var LessonMabagerClass = function () {
        self.$http = $http;
        self._cashPool = {};
    };
    /** METHODS */
    LessonMabagerClass.prototype = {
            ...
            self.$http.post("'api.php?controller=school_rooms&action=item&name=" + name) //wrong address
                .then(function (responce) {
                    if (responce.status >= 200 && responce.status < 300) { //JSON.parse(responce)
                        var instance = new Lesson(responce.data);
