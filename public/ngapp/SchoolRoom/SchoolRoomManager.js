...
function SchoolRoomManager() {
    var self = this;
    var SchoolManagerClass = function ($http, $q) {
        self.$http = $http;
        self._cashPool = {};
    };

    /** METHODS */
    SchoolManagerClass.prototype = {
            ...
            self.$http.post("'api.php?controller=school_rooms&action=item&name=" + name) //wrong address
                .then(function (responce) {
                    if (responce.status >= 200 && responce.status < 300) { //JSON.parse(responce)
                        var instance = new SchoolRoom(responce.data);
