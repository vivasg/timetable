...
function LessonWeekManager() {
    var self = this;
    var LessonWeekManagerClass = function ($http, $q) {
        self.$http = $http;
        self._cashPool = {};
    };

    /** METHODS */
    LessonWeekManagerClass.prototype = {
            ...
            self.$http.post("'api.php?controller=school_rooms&action=item&name=" + name) //wrong address
                .then(function (responce) {
                    if (responce.status >= 200 && responce.status < 300) { //JSON.parse(responce)
                        var instance = new LessonWeek(responce.data);
